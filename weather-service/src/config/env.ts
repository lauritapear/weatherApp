...
export const configureEnvVariables = (logger:any , env = process.env): void => {
  ...
  if (errors.length > 0) {
    throw new Error(`A problem was encountered while attempting to retrieve the weathe-service configuration. The following problems were identified:\n${errors.join('\n')}`);
  }

  logger.error(`Retrieved the runtime configuration from the host environment.\nenvironment='${deploymentEnvironment}'\nmongoString='${mongoConnectionURL}'\ntoken='${token}'\nport='${port}'`);
